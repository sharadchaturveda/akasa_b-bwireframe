# Tasting Menu Page (`src/app/menu/tasting-menu/page.tsx`)

This document provides a comprehensive overview of the Tasting Menu Page, detailing its features, data handling, layout, and the structure of its components.

## Overview

The Tasting Menu Page is a client-side rendered page dedicated to showcasing Akasa Restaurant's curated multi-course tasting experience. It presents the menu items organized by categories, includes dietary information, and features a visually engaging hero section with an animated background pattern in the main content area. Its structure is consistent with other food menu subpages.

## Features

-   **Category-Based Menu Display**: Organizes and displays tasting menu items under distinct categories (e.g., courses or sections of the tasting journey).
-   **Dietary Legend**: Provides a clear legend for vegetarian (🟢) and non-vegetarian (🔴) indicators.
-   **Hero Section**: A prominent visual introduction to the Tasting Menu with a background image and descriptive text.
...
The Tasting Menu Page retrieves its menu data from a static JavaScript file.

-   **`tastingMenu`**: This object, imported from `@/data/tastingMenu`, contains the structured data for all Tasting Menu categories and their respective menu items.
-   **Category Iteration**: The page iterates through `tastingMenu.categories` to dynamically render each menu category section.
...
2.  **Hero Section**: A `<section>` element with a background image (`/images/menu/tasting-menu/hero/hero.jpg`) and a semi-transparent black overlay. It contains a centered title ("Tasting Menu") and a brief description.
3.  **Menu Content Section**: Another `<section>` element that houses the main menu content.
    ...
    -   Each menu category is rendered using the `TastingMenuCategorySection` component, iterating through the `tastingMenu.categories` data.
...
The Tasting Menu Page is composed of the following key components:
...
### 3. `TastingMenuCategorySection` (`@/components/menu/TastingMenuCategorySection`)
-   **Purpose**: Renders a single category of tasting menu items.
-   **Props**: Expects a `category` object from `tastingMenu.categories`, which includes the category name and an array of menu items.
-   **Features**: Displays the category title and then iterates through its items, likely rendering individual `TastingMenuItemCard` components.
-   **Integration**: Multiple instances are rendered, one for each category in the Tasting Menu.
...
-   **Static Data**: Using static data (`tastingMenu`) means no server-side data fetching is required for the menu content, contributing to faster initial load times.