...
jest.mock('@/components/menu/MenuCategorySection', () => {
  return function MockMenuCategorySection({ category }: { category: { category_name: string } }) {
    return (
...
jest.mock('@/components/mobile/MobileMenuCategorySection', () => {
  return function MockMobileMenuCategorySection({ category }: { category: { category_name: string } }) {
    return (
...
describe('ALaCarteMenuPage', () => {
  ...
  it('optimizes images on mount', () => {
    ...
    const mockImages = [
      ...
      { complete: false, classList: { add: jest.fn() }, onload: null as unknown as () => void },
    ];
