...
import MithaiMenuItemCard from './MithaiMenuItemCard';
import CategoryNotes from './CategoryNotes';

interface MithaiCategorySectionProps {
  category: MenuCategory;
}

const MithaiCategorySection = memo(function MithaiCategorySection({ category }: MithaiCategorySectionProps) {
  return (
    <div className="mb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {category.items.map((item, index) => (
          <MithaiMenuItemCard key={`${category.category_name}-${index}`} item={item} />
        ))}
      </div>
    </div>
  );
...
export default MithaiCategorySection;
