...
import MobileBarBiteMenuItemCard from './MobileBarBiteMenuItemCard';
import MobileCategoryNotes from './MobileCategoryNotes';

interface MobileBarBiteCategorySectionProps {
  category: MenuCategory;
}

const MobileBarBiteCategorySection = memo(function MobileBarBiteCategorySection({ category }: MobileBarBiteCategorySectionProps) {
  return (
    <div className="mb-10">
      ...
      <div className="flex flex-col gap-3">
        {category.items.map((item, index) => (
          <MobileBarBiteMenuItemCard key={`${category.category_name}-${index}`} item={item} />
        ))}
      </div>
    </div>
  );
...
export default MobileBarBiteCategorySection;
